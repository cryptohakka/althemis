...
async function tickProvider(role, frSig, dec) {
  ...
  console.log(`[diag] ${role} calling getProviderBondRate...`);
  const rateBps    = await getProviderBondRate(agent.address);
  console.log(`[diag] ${role} getProviderBondRate OK: ${rateBps}`);
  const bondNeeded = (budget * rateBps) / 10000n;
  console.log(`[diag] ${role} calling getFreeBalance...`);
  const free       = await getFreeBalance(agent.address);
  console.log(`[diag] ${role} getFreeBalance OK: ${free}`);
  if (free < bondNeeded) {
